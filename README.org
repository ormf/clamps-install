...
**** Testing Lilypond
     ...
     To test the lilypond configuration, enter the following into the
     REPL:
     ...
     If the evince pdf viewer pops up with the display of a Middle c
     quarter note, the lilypond configuration is working.

***** Troubleshooting the lilypond configuration

      - make sure, the programs =/usr/bin/lilypond= and
        =/usr/bin/evince= exist, otherwise install them at these
        locations or edit the file =~/.fomus= to reflect their install
        locations on the right had side of the following lines in that
        file:

        =:lilypond-exe "/usr/bin/lilypond"=
